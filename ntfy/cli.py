...
def watch_pid(args):
    ...
    duration = time() - start_time
    message = 'PID[{}]: "{}" finished in {:d}:{:02d} minutes'.format(
        p.pid,
        ' '.join(cmd),
        *map(int, divmod(duration, 60)))
    return message, ret
...
def auto_done(args):
    if args.longer_than:
        print('export AUTO_NTFY_DONE_LONGER_THAN=-L{}'
              .format(args.longer_than))
...
class BackendOptionAction(argparse.Action):
    ...
    def __call__(self, parser, args, values, option_string=None):
        if self.dest == 'backend':
            ...
            if args.backend is None:  # pragma: no cover
                args.backend = []
            args.backend.append(values)
        elif self.dest == 'option':
            if args.option is None:  # pragma: no cover
                args.option = {}
            args.option.setdefault(self.__class__.backend,
                                   {})[values[0]] = values[1]
        else:
            raise Exception("BackendOptionAction only "  # pragma: no cover
                            "supports dest of 'backend' and 'option'")
...
def parse_args(args=None):
    ...
    done_parser.add_argument(
        ...
        nargs=3,
        help="Format and send cmd, retcode & duration instead of running"
             "command. Used internally by shell-integration")
